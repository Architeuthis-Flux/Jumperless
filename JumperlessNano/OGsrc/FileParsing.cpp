...
int parseRaw(int connectOrClear) {
  ...
  while (Serial.available() == 0) {
    if (millis() - humanTime == 300 && shown == 0) {
      Serial.print(
          "\th/H [clear/CONNECT], chip [A-L/0-11], X [0-15], Y [0-7]\n\n\r");
...
void savePreformattedNodeFile(int source, int slot, int keepEncoder) {
  ...
  if (source == 1) {
    ...
    while (Serial1.available() == 0 || Serial1.read() == 'f') {
    }
...
//       }
//     }
// //char uart[30] = " 116-70, 117-71, ";
//     // for (int i = 0; i < sizeof(uart); i++) {
//     //   buffer[readCount] = uart[i];
//     //   readCount++;

//     // }

//     for (int i = 0; i < readCount; i++) {
//       nodeFile.print(buffer[i]);
//     }
...
//     Serial.println(" ");
//nodeFile.print("  117-D1, 116-D0, ");
    // while (Serial1.available() > 0) {
    //   // nodeFile.write(Serial1.read());
    //   nodeFileString.read(Serial1);
