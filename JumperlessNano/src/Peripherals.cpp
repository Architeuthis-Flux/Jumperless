...
void setDac0voltage(float voltage, int save, int saveEEPROM) {
  ...
  setDac0_5Vvoltage(voltage);
  return;

}

void setDac0voltage(uint16_t inputCode) {
  setDac0_5VinputCode(inputCode);
  return;
}
...
void setDac1voltage(float voltage, int save, int saveEEPROM) {
  ...
  setDac1_8Vvoltage(voltage);
  return;
}

void setDac1voltage(uint16_t inputCode) {
  setDac1_8VinputCode(inputCode);
  return;
}
...
uint8_t csToPin[16] = {6,7,8,9,10,11,12,13,20,21,22,23};
...
void setCSex(int chip, int value) {
  ...
  if (value > 0) {
    digitalWrite(csToPin[chip], HIGH);
  } else {
    digitalWrite(csToPin[chip], LOW);
  }
}
...
for (int i = 0; i < 8; i++) {
  ...
  if (showADCreadings[i] > 0 && showADCreadings[i] <= numberOfNets) {
    ...
    if (displayMode == 0 || numberOfShownNets > MAX_NETS_FOR_WIRES) {
      lightUpNet(showADCreadings[i], -1, 1, brightness, 0,  color);
    }
...
int shift = 228;
 ...
 // Serial.println(hsv.h);
  ...
  color = packRgb(rgb.r, rgb.g, rgb.b);
