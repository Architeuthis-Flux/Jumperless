...
int justReadProbe(void)
    {
        ...
        int pressed = 0;
    int reading = -1;
    rawOtherColors[1] = 0xa500e8;
    showLEDsCore2 = 1;
    while (Serial.available() == 0 && reading == -1)
        {
        delayMicroseconds(38000);

        connectedRowsIndex = 0;

        reading = scanRows(ADC0_PIN);


        if (reading == -18 && (millis() - probingTimer > 500) && checkProbeButton() == 1 && (millis() - probeButtonTimer) > 1000)
            {
            if (longShortPress(650) == 1)
                {
...
if (pressed == 0)
    {
    leds.setPixelColor(nodesToPixelMap[reading], rainbowList[0][0], rainbowList[0][1], rainbowList[0][2]);
    showLEDsCore2 = 2;
...
// int reading2 = -1 ; //scanRows(ADC0_PIN);
// probeTimeout = millis();
// int countMissed = 0;

// while (Serial.available() == 0 && countMissed < 5)
//     {
//     delayMicroseconds(38000);
//     connectedRowsIndex = 0;
//     while (reading2 == -1 && countMissed < 5)
//         {
//         reading2 = scanRows(ADC0_PIN);
//         countMissed++;
//         }
...
int selectFromLastFound(int print)
    {
